...
import type { Client, ClientCase, Company, ClientPayload } from '../../types';
import { CaseModal } from './CaseModal';
import { apiService } from '../../services/api';

interface IndividualsSectionProps {
  availableCompanies: Company[];
  onRelationsChanged?: () => Promise<void> | void;
}
...
function matchesClient(c: Client, q: string): boolean {
  ...
  const linkedCompanies = (c.companies ?? []).some((link) => {
    const ln = (link.name ?? '').toLowerCase();
    const le = (link.email ?? '').toLowerCase();
    const lp = (link.phone ?? '').toLowerCase();
    const lpDigits = (link.phone ?? '').replace(/\D/g, '');
    return (
      ln.includes(ql) ||
      le.includes(ql) ||
      lp.includes(ql) ||
      (!!qDigits && lpDigits.includes(qDigits))
    );
  });
...
export function IndividualsSection({ availableCompanies, onRelationsChanged }: IndividualsSectionProps) {
  ...
  const handleSubmitClient = async (clientData: ClientPayload) => {
    if (editingClient) {
      await updateClient(editingClient.id, clientData);
    } else {
      await createClient(clientData);
    }
    await notifyRelationsChanged();
  };
  ...
  const notifyRelationsChanged = async () => {
    if (!onRelationsChanged) return;
    try {
                            ...
                            <div>
                              ...
                              {client.companies && client.companies.length > 0 && (
                                <div className="mt-1 flex flex-wrap gap-2">
                                  {client.companies.map((link) => (
                                    <span
                                      key={`${client.id}-table-${link.id}`}
                                      className="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] bg-blue-50 text-blue-700 rounded-full">
                                      {link.name}
                                    </span>
                                  ))}
                                </div>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {client.companies && client.companies.length > 0 && (
                          <div className="mt-1 flex flex-wrap gap-2">
                            {client.companies.map((link) => (
                              <span
                                key={`${client.id}-card-${link.id}`}
                                className="inline-flex items-center gap-1 px-2.5 py-1 text-xs bg-blue-50 text-blue-700 rounded-full">
                                {link.name}
                              </span>
                            ))}
                          </div>
