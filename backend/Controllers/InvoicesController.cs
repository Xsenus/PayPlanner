...
public class InvoicesController : ControllerBase
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            ModelState.AddModelError(nameof(status), statusError ?? "Invalid status value.");
            return ValidationProblem(ModelState);
        }

        var query = BuildInvoiceQuery();
        query = ApplyFilters(query, from, to, statusValue, clientId, responsibleId, search);
    ...
    private static bool TryParseStatus(string? raw, out PaymentStatus? value, out string? error)
    {
        value = null;
        error = null;
        ...
        if (Enum.TryParse<PaymentStatus>(raw, true, out var parsed))
        {
            value = parsed;
            return true;
        }

        error = $"The value '{raw}' is not valid. Допустимые значения: {string.Join(", ", Enum.GetNames<PaymentStatus>())}.";
        return false;
    }
