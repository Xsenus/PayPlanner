...
public class UserActivityService : IUserActivityService
{
    ...
    private static readonly TimeSpan RetryInitialDelay = TimeSpan.FromMilliseconds(50);
    ...
    public async Task<UserActivityLog?> TryWriteAsync(ActivityLogEntry entry, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            while (true)
            {
                ...
                catch (DbUpdateException dbEx) when (IsSqliteBusy(dbEx) && attempt < MaxRetryCount - 1)
                {
                    attempt++;
                    var delay = TimeSpan.FromMilliseconds(RetryInitialDelay.TotalMilliseconds * Math.Pow(2, attempt - 1));
                    _logger.LogWarning(dbEx, "Попытка {Attempt}/{Max} записи активности завершилась блокировкой БД, повтор", attempt, MaxRetryCount);
                    await Task.Delay(delay, cancellationToken);
                }
                catch (DbUpdateException dbEx) when (IsSqliteBusy(dbEx))
                {
                    _logger.LogError(dbEx, "Не удалось сохранить журнал активности пользователя из-за блокировки БД");
                    return null;
                }
                catch (SqliteException sqliteEx) when (IsSqliteBusy(sqliteEx) && attempt < MaxRetryCount - 1)
                {
                    attempt++;
                    var delay = TimeSpan.FromMilliseconds(RetryInitialDelay.TotalMilliseconds * Math.Pow(2, attempt - 1));
                    _logger.LogWarning(sqliteEx, "Попытка {Attempt}/{Max} записи активности завершилась блокировкой БД, повтор", attempt, MaxRetryCount);
                    await Task.Delay(delay, cancellationToken);
                }
                catch (SqliteException sqliteEx) when (IsSqliteBusy(sqliteEx))
                {
                    _logger.LogError(sqliteEx, "Не удалось сохранить журнал активности пользователя из-за блокировки БД");
                    return null;
                }
            }
        }
    ...
    private static bool IsSqliteBusy(Exception ex)
    {
        ...
        {
            return sqliteEx.SqliteErrorCode is (int)SqliteErrorCode.Busy or (int)SqliteErrorCode.Locked;
        }

        if (ex is DbUpdateException { InnerException: not null } dbUpdate)
        {
            return IsSqliteBusy(dbUpdate.InnerException);
        }
