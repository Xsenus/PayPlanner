...
namespace PayPlanner.Api.Services
{
    ...
    public static class SeedDataService
    {
        ...
        public static async Task SeedAsync(PaymentContext context, bool seedClientsAndPayments = false)
        {
            ...
            await SeedRolePermissionsAsync(context);       // права ролей
        ...
        private static async Task SeedDictionariesAsync(PaymentContext context)
        {
            ...
            if (!await context.PaymentStatuses.AnyAsync())
            {
                var paymentStatuses = new[]
                {
                    ...
                    new PaymentStatusEntity { Name = "В обработке", Description = "Платёж принят и обрабатывается",    ColorHex = "#3B82F6" },
                    new PaymentStatusEntity { Name = "Отменено",    Description = "Оплата отменена или отозвана",      ColorHex = "#6B7280" }
                };
            ...
            await EnsurePaymentStatusAsync(context, "В обработке", "#3B82F6", "Платёж принят и обрабатывается");
            await EnsurePaymentStatusAsync(context, "Отменено", "#6B7280", "Оплата отменена или отозвана");
        }
        ...
        private static readonly string[] PermissionSections = new[]
        {
            "calendar",
            "reports",
            "calculator",
        ...
        private static RolePermission CreateDefaultPermission(int roleId, string section)
        {
        ...
        {
            if (!await context.TableExistsAsync("RolePermissions"))
            {
                return;
            }
            ...
            var existingSet = new HashSet<(int RoleId, string Section)>(
                existing.Select(x => (x.RoleId, x.Section.ToLowerInvariant()))
            );
            ...
            foreach (var role in roles)
            {
                ...
                {
                    var key = (role.Id, section.ToLowerInvariant());
                    if (existingSet.Contains(key))
                    {
                        continue;
                    }

                    toInsert.Add(CreateDefaultPermission(role.Id, section));
                }
            }

            if (toInsert.Count > 0)
            {
                context.RolePermissions.AddRange(toInsert);
                await context.SaveChangesAsync();
            }
        }
        ...
        private static async Task SeedClientsCasesPaymentsAsync(PaymentContext context)
        {
            ...
            var paymentsToEnsure = new[]
            {
                ...
                new
                {
                    ...
                    Description = "Оплата за поставку обрабатывается",
                    IsPaid = false,
                    PaidDate = (DateTime?)null,
                ...
                new
                {
                    ...
                    Description = "Этап 1 — НИР (счёт отменён)",
                    IsPaid = false,
                    PaidDate = (DateTime?)null,
                    DealType = "Проект",
                    IncomeType = "Прочие доходы",
